...
export const scrapeGetAuthor = async (novel_title) => {
        ...
        .then(async (res) => {
            ...
            $(".ul-list1.ul-list1-2.ss-custom>.li-row").map(
                async (i, novel) => {
                    ...
                    const genre = $(novel)
                        .find("div.desc > div:nth-child(2) > div.right")
                        .text()
                        .split(",")
                        .map((item) => item.trim());
...
const genres = [];
export const scrapeGetAllGenres = () =>
    getData(novelHomepage).then((res) => {
        const $ = load(res);
...
export const scrapeGetGenreTitles = async (genre_title, page_index) => {
...
export const scrapeGetChapter = async (novel_title, ch_no) => {
...
//RECOMMENDATIONS
export const scrapeGetKnnRecommendation = async (novel_id) => {
    try {
        ...
        async function fetchAllNovels() {
            const query =
                "select scrapeGt.id as novel_id, group_concat(gnt.genre_id separator ',') as genre from novel_tbl nt left join genre_novel_tbl gnt on nt.id =gnt.novel_id group by nt.id,nt.title ;";
